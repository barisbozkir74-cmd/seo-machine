...
import { getClusterMetrics } from '@/lib/monitoring/aggregation'
...
export async function GET(request: NextRequest) {
  ...
  if (!project.gsc_property_url) {
    return NextResponse.json({ clusters: [], gscConnected: false })
  }

  try {
    const clusters = await getClusterMetrics(supabase, projectId, period)
    return NextResponse.json({ clusters, gscConnected: true, period })
  } catch (err) {
